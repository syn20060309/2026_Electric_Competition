...
{
    assert(LapFinish_Update(first_clear_ms, 100U, true, 0x0FU) ==
    ...
    assert(LapFinish_Update(first_clear_ms + 100U, 200U, true, 0x0FU) ==
        LAP_FINISH_EVENT_NONE);
...
static void test_start_line_requires_valid_continuous_clear(void)
{
    ...
    assert(LapFinish_Update(1010U, 10U, true, 0xFFU) ==
        LAP_FINISH_EVENT_NONE);
    assert(LapFinish_Update(1020U, 20U, true, 0x0FU) ==
        LAP_FINISH_EVENT_NONE);
    assert(LapFinish_Update(1119U, 119U, false, 0x00U) ==
        LAP_FINISH_EVENT_NONE);
    assert(LapFinish_Update(1120U, 120U, true, 0x0FU) ==
        LAP_FINISH_EVENT_NONE);
    assert(LapFinish_Update(1219U, 219U, true, 0x1FU) ==
        LAP_FINISH_EVENT_NONE);
    assert(LapFinish_GetState() == LAP_STATE_LEAVING_START);
    assert(LapFinish_Update(1220U, 220U, true, 0x0FU) ==
        LAP_FINISH_EVENT_NONE);
    assert(LapFinish_Update(1319U, 319U, true, 0x0FU) ==
        LAP_FINISH_EVENT_NONE);
    assert(LapFinish_GetState() == LAP_STATE_LEAVING_START);
    assert(LapFinish_Update(1320U, 320U, true, 0x0FU) ==
        LAP_FINISH_EVENT_NONE);
    assert(LapFinish_GetState() == LAP_STATE_RUNNING);
}
...
static void test_finish_requires_time_validity_and_two_consecutive_samples(void)
{
    ...
    assert(LapFinish_Update(threshold - 1U, threshold - 1U, true, 0xFFU) ==
        LAP_FINISH_EVENT_NONE);
    assert(LapFinish_Update(threshold, threshold, true, 0x0FU) ==
        LAP_FINISH_EVENT_NONE);
    assert(LapFinish_Update(threshold + 1U, threshold + 1U, true, 0x1FU) ==
        LAP_FINISH_EVENT_NONE);
    ...
    assert(LapFinish_Update(threshold + 3U, threshold + 3U, true, 0x1FU) ==
        LAP_FINISH_EVENT_NONE);
    assert(LapFinish_GetConfirmCount() == 1U);
    assert(LapFinish_Update(threshold + 4U, threshold + 4U, true, 0x0FU) ==
        LAP_FINISH_EVENT_NONE);
    assert(LapFinish_GetConfirmCount() == 0U);
...
static void test_five_through_eight_active_can_finish(void)
{
    assert_active_count_can_finish(0x1FU);
...
static void test_clear_confirmation_handles_clock_wrap(void)
{
    ...
    assert(LapFinish_Update(UINT32_MAX - 50U, 30U, true, 0x1FU) ==
        LAP_FINISH_EVENT_NONE);
    assert(LapFinish_Update(UINT32_MAX - 40U, 40U, true, 0x0FU) ==
        LAP_FINISH_EVENT_NONE);
    assert(LapFinish_Update(58U, 139U, true, 0x0FU) ==
        LAP_FINISH_EVENT_NONE);
    assert(LapFinish_GetState() == LAP_STATE_LEAVING_START);
    assert(LapFinish_Update(59U, 140U, true, 0x0FU) ==
        LAP_FINISH_EVENT_NONE);
    assert(LapFinish_GetState() == LAP_STATE_RUNNING);
}
...
int main(void)
{
    ...
    test_five_through_eight_active_can_finish();
