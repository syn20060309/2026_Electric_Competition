 ...
 * Notes: None
******************************************************************/
char MPU6050_WriteReg(uint8_t addr,uint8_t regaddr,uint8_t num,const uint8_t *regdata)
{
    uint16_t i = 0;
        I2C_Start();
        Send_Byte((addr<<1)|0);
...
char MPU6050_Init(void)
{
    ...
    data = 0x80U;
    if (MPU6050_WriteReg(0x68, MPU6050_RA_PWR_MGMT_1, 1, &data) != 0) {
        return 1;
    }
    delay_ms(100);
    ...
    data = 0x00U;
    if (MPU6050_WriteReg(0x68, MPU_INT_EN_REG, 1, &data) != 0 ||
        MPU6050_WriteReg(0x68, MPU_USER_CTRL_REG, 1, &data) != 0 ||
        MPU6050_WriteReg(0x68, MPU_FIFO_EN_REG, 1, &data) != 0) {
        return 1;
    }

    data = 0x80U;
    if (MPU6050_WriteReg(0x68, MPU_INTBP_CFG_REG, 1, &data) != 0) {
        return 1;
    }
    ...
    data = 0x00U;
    if (MPU6050_WriteReg(0x68, MPU_PWR_MGMT2_REG, 1, &data) != 0) {
        return 1;
    }
    if (MPU_Set_Rate(50) != 0) {
        return 1;
